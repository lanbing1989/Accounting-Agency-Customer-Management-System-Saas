...
if ($isEdit) {
    // 只允许编辑本租户模板
    $stmt = $db->prepare("SELECT * FROM contract_templates WHERE id=:id AND tenant_id=:tenant_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isEdit) {
        $stmt = $db->prepare("UPDATE contract_templates SET name=:n, content=:c WHERE id=:id AND tenant_id=:tenant_id");
        ...
        $stmt = $db->prepare("INSERT INTO contract_templates (tenant_id, name, content, created_at) VALUES (:tid, :n, :c, :t)");
