    ...
    if (($handle = fopen($file, 'r')) !== false) {
        ...
        if (!$header || $header[0] != '客户名称') {
            ...
            while (($data = fgetcsv($handle)) !== false) {
                ...
                // 判断唯一性（仅本租户）
                $stmt = $db->prepare("SELECT COUNT(*) as cnt FROM contracts WHERE client_name = :client_name AND tenant_id=:tenant_id");
                ...
                // 插入
                $stmt = $db->prepare("INSERT INTO contracts (tenant_id, client_name, contact_person, contact_phone, contact_email) VALUES (:tenant_id, :client_name, :contact_person, :contact_phone, :contact_email)");
                $stmt->bindValue(':tenant_id', $_SESSION['tenant_id'], PDO::PARAM_INT);
                $stmt->bindValue(':client_name', $client_name, PDO::PARAM_STR);
